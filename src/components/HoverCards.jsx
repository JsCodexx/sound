...
import { Plans } from '@/api/Sounds/plans';
...
export default function HoverCards() {
    ...
    async function plandata() {
        try {
            showloading()
            const resData = await Plans()
            const minData = resData.data.items
            setPlan(minData)
            console.log(resData, "plan")
        } catch (error) {
                ...
                <div className='bg-black hover:bg-[#D74664]  px-6 text-start py-6 rounded-lg transition-all duration-700 ease-in-out hover:-translate-y-2 hover:scale-[1.1] '>
                    <div className='text-white   pt-7 flex flex-col group gap-5 '>
                        ...
                        <>
                            ...
                            <p className='text-4xl text-[#FFFFFF] font-[Manrope] font-semibold'>${data.price} <span className='text-sm text-gray-300 font-light'>/Month</span></p>
