...
export default function PackCard({ src, id, description, img, num, title, ChangeRow, showArrow, variant }) {
    return (
        <div>
            {variant === "label" ?
                <div className={` bg-[#E9E9E9] w-full h-auto flex justify-between ${ChangeRow ? 'bg-[#FFFFFF]' : ''}`}>
                    ...
                    <div className='flex gap-20'>
                        ...
                        <div className='flex  mt-3'>
                            <div className="flex gap-3">
                                {description.map((tag) => (
                                    <button className="w-[79px] h-[32px] rounded-2xl text-sm text-[#4F4F4F] bg-[#FFFFFF]">
                                        {tag}
                                    </button>
                                ))}
                            </div>
                    ...
                    <div className='flex gap-2 pr-10 pt-4'>
                        ...
                        {showArrow && <img src={showArrow} className='h-3 w-3 mt-3' />}
                    </div>
